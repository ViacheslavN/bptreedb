...
namespace bptreedb
{
	...
	public:
		...
		TZstdEncoder(CommonLib::IAllocPtr& pAlloc, TCompressorParamsPtr pParams) : m_nCount(0), m_pCompParams(pParams)
		{
		...
		uint32_t Encode(const TValueMemSet& vecValues, CommonLib::IWriteStream *pStream, uint32_t maxCompSize)
		{
			try
			{
				...
				CommonLib::CZstdCompressionStream zStream(m_pCompParams->m_compressLevel);
				zStream.AttachOut(pMemStream->Buffer() + pStream->Pos(), streamSize);

				for (uint32_t i = 0, sz = (uint32_t)vecValues.size(); i < sz; ++i)
				{
					zStream.AttachIn((const byte_t*)&vecValues[i], sizeof(TValue));
					while (zStream.GetAvailIn() != 0)
					{
						if (zStream.GetAvailOut() == 0)
						{
							/*	unsigned pending;
								int bits;
								zStream.DeflatePending(&pending, &bits);

								uint32_t totalIn = zStream.GetTotalIn();
								uint32_t compressSize = streamSize + pending;

								double part = (double)compressSize / (double)streamSize;
								double proc = (double)streamSize*100 / (double)compressSize;

								int count = (vecValues.size() * proc) / 100;

								return (uint32_t)(vecValues.size() - count);*/
		...
		void Decode(uint32_t nCount, TValueMemSet& vecValues, CommonLib::IReadStream *pStream, uint32_t nCompSize)
		{
			try
			{
				...
				CommonLib::CZstdDecompressionStream zStream;
