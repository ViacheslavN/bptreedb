...
namespace bptreedb
{
	...
	int32_t CompressorParams::GetIntParam(const std::string& name, int32_t defValue) const
	{
		return GetParam<int32_t>(name, m_IntParams, defValue);
	}

	double CompressorParams::GetDoubleParam(const std::string& name, double defValue) const
	{	
		return GetParam<double>(name, m_DoubleParams, defValue);
	}

	void  CompressorParams::SetIntParam(const std::string& name, int32_t value)
	{
		m_IntParams[name] = value;
	}

	void  CompressorParams::SetDoubleParam(const std::string& name, double value)
	{
